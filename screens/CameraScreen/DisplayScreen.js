import { View, Text, SafeAreaView, TouchableOpacity, Share, StyleSheet, Image, Alert, Dimensions } from 'react-native';
...
export default function DisplayScreen() {
    ...
    if (imageUrl === 0) {
        return (
            <View style={[STYLES.container, { padding: 30, justifyContent: 'center' }]}>
                <View style={{ marginTop: 30, }}>
                    <TouchableOpacity onPress={() => navigation.navigate('HomeScreen')}>
                        <Icon name='caret-up' size={40} color={'white'} />
                    </TouchableOpacity>
                </View>
                <View style={{ alignItems: 'center', flex: 1, flexDirection: 'column', padding: 30, marginTop: 100, }}>
                    <Icon name='images' size={100} color={'white'} style={{ marginBottom: 10 }} />
                    <Text style={{ fontWeight: 'bold', fontSize: 27, color: 'white', marginBottom: 10, justifyContent: 'center', }}>Send your first Locket</Text>
                    <Text style={{ fontWeight: 'bold', fontSize: 22, color: 'gray', justifyContent: 'center', }}>Your picture will appear on your friends' home screens</Text>
                </View>
            </View>
        );
    }
    ...
    const onActionButtonPress = () => {
        Alert.alert('Action', 'Choose an action for the photo:', [
    ...
    const renderItem = (item, index) => {
        if (!item.caption) {
            return (
                <View style={{ width: screenWidth, height: screenHeight, marginTop: 80 }}>
        ...
        return (
            <View key={index} style={{ width: screenWidth, height: screenHeight, marginTop: 80 }}>
                <View style={styles.postContainer}>
                    <Image source={{ uri: item.url }} style={styles.image} />
    ...
    return (
        <SafeAreaView style={[STYLES.container, { flex: 1, }]}>
            <View style={[styles.header, { marginTop: 30, alignItems: 'center' }]}>
                ...
                <TouchableOpacity onPress={toggleFriendList} style={styles.toggleButton}>
